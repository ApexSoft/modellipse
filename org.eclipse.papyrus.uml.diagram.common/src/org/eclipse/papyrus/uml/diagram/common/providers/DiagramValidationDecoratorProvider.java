...
/**
 * A provider giving the decorations to use in the diagram validation.
 * 
...
public class DiagramValidationDecoratorProvider extends AbstractProvider implements IDecoratorProvider {
	...
	private static final String MARKER_TYPE = "es.cv.gvcase.mdt.common.validation.diagnostic";
		...
		PlatformUI.getWorkbench().getDisplay().asyncExec(new Runnable() {
			...
			public void run() {
				try {
					...
					Activator.getDefault().logError("Decorator refresh failure", e); //$NON-NLS-1$
				}
			}
		});
	}
	...
	public static class StatusDecorator extends AbstractDecorator {
		...
		public StatusDecorator(IDecoratorTarget decoratorTarget) {
			...
			try {
				...
				Activator.getDefault().logError("ViewID access failure", e); //$NON-NLS-1$			
			}
		}
		...
		public void activate() {
			...
			List<Object> list = (List)allDecorators.get(viewId);
			if(list == null) {
	...
	static class MarkerObserver implements IFileObserver {
		...
		private String getType(IMarker marker) {
			try {
				...
				Activator.getDefault().logError("Validation marker refresh failure", e); //$NON-NLS-1$
				return ""; //$NON-NLS-1$
			}
		}
	}
